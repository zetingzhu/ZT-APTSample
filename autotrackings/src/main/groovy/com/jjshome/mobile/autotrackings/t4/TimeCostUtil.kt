...
object TimeCostUtil {
    ...
    fun recordMethodEnd(thresholdTime: Int, methodName: String, clz: Any?) {
        println(
            "$TAG\t methodName=>$methodName thresholdTime=>$thresholdTime method=>recordMethodEnd"
        )
            ...
            try {
                if (METHODS_MAP.containsKey(methodName)) {
                    ...
                    if (costTime >= thresholdTime) {
                        ...
                        println(
                            TAG+"\t methodName=>$methodName threadNam=>$threadName thresholdTime=>$thresholdTime costTime=>$costTime"
                        )
                    }
                }
            } catch (ex: Exception) {
